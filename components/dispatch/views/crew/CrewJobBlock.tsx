"use client";

import { useDraggable } from "@dnd-kit/core";
import { CSS } from "@dnd-kit/utilities";
import type { DispatchJob } from "@/types/dispatch";
import { cn } from "@/lib/utils";
import { CELL_WIDTH, getJobPosition } from "../timeline/constants";
import { statusStyles } from "./crew-helpers";

interface CrewJobBlockProps {
  job: DispatchJob;
  onJobClick: (job: DispatchJob) => void;
  lane?: number;
  laneHeight?: number;
}

export function CrewJobBlock({ job, onJobClick, lane = 0, laneHeight = 40 }: CrewJobBlockProps) {
  const { left, width } = getJobPosition(job, CELL_WIDTH);
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({
    id: job.id,
    data: { job },
  });

  const style: React.CSSProperties = {
    position: "absolute",
    top: 8 + lane * laneHeight,
    height: laneHeight - 8,
    left,
    width: Math.max(width - 4, 96),
    zIndex: isDragging ? 100 : 1,
    transform: transform ? CSS.Translate.toString(transform) : undefined,
  };

  return (
    <div
      ref={setNodeRef}
      style={style}
      {...listeners}
      {...attributes}
      onClick={() => { if (!isDragging) onJobClick(job); }}
      className={cn(
        "border-l-3 rounded-r-md touch-none overflow-hidden select-none",
        "flex flex-col justify-center px-2",
        isDragging && "opacity-80 ring-2 ring-blue-400 cursor-grabbing",
        !isDragging && "cursor-grab hover:brightness-95",
        statusStyles[job.status]
      )}
    >
      <div className="text-xs font-medium text-gray-800 truncate">{job.title}</div>
      <div className="text-[10px] text-gray-600 truncate">{job.customerName}</div>
    </div>
  );
}
