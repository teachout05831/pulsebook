...
import { ServicePickerModal } from "../pickers/ServicePickerModal";
import { MaterialPickerModal } from "../pickers/MaterialPickerModal";
import { CustomLineItemModal } from "../pickers/CustomLineItemModal";
import { calculateTotals } from "../../utils/calculateTotals";
import { FeeTogglesSection } from "./FeeTogglesSection";
import { HourlyRangeDisplay } from "../shared/HourlyRangeDisplay";
import type { EstimateDetail, EstimateLineItem, LineItemCategory, AppliedFee } from "@/types/estimate";
...
export function PricingCard({ estimate, categories, lineItems, onAddItem, onRemoveItem, appliedFees = [], onUpdateFees, onOpenPayments }: Props) {
  ...
  const totals = calculateTotals({
    lineItems, resources: estimate.resources || {}, pricingModel: estimate.pricingModel || "flat",
    taxRate: estimate.taxRate || 0, depositType: estimate.depositType as "percentage" | "fixed" | null,
    depositValue: estimate.depositAmount || 0, depositPaid: estimate.depositPaid || 0,
    appliedFees,
  });
  ...
  const hasDeposit = estimate.depositType && estimate.depositAmount;
      ...
      <Card className="overflow-hidden">
        <CardContent className="p-4 space-y-1">
          ...
          <div className="pt-3 space-y-1">
            ...
            <FeeTogglesSection fees={appliedFees} isHourly={estimate.pricingModel === "hourly"} onToggle={handleFeeToggle} />
            ...
            <div className="flex justify-between text-[13px]"><span className="font-semibold">Estimated Total</span><span className="font-semibold">{fmt(totals.total)}</span></div>
            {hasDeposit && (
              <>
        ...
        {/* Blue total bar */}
        <div className="bg-blue-800 text-white rounded-b-xl px-5 py-3">
          <div className="flex justify-between text-[13px] mb-1">
            <span>{estimate.serviceType || "Service"}</span>
            <span>{fmt(totals.total)}</span>
          </div>
          {estimate.pricingModel === "hourly" && estimate.resources ? (
            <HourlyRangeDisplay resources={estimate.resources} rate={hourlyRate} total={totals.total} textColor="white" />
          ) : (
