...
export function useServiceCatalog(initialItems: ServiceCatalogItem[]) {
  ...
  const createItem = useCallback(
    async (input: { name: string; description?: string; category: string; pricingModel: string; defaultPrice: number; unitLabel?: string; isTaxable?: boolean }) => {
      const res = await fetch("/api/service-catalog", {
        method: "POST",
