...
export async function addCrewMember(crewId: string, teamMemberId: string, isDefault = true) {
  ...
  const { data, error } = await supabase
    .from("crew_members")
    .upsert(
      { crew_id: crewId, team_member_id: teamMemberId, is_default: isDefault },
      { onConflict: "crew_id,team_member_id" }
    )
    .select("id, crew_id, team_member_id, is_default, created_at")
    .single();
